...
from App.routers.commands import (future_settlements_cmd, customer_remain_cmd, weekly_wage_cmd,
                                  ...
                                  daily_portfo_composition, credit_kpi_cmd)
...
async def read_daily_portfo_composition_data(db_session: Session = Depends(get_postgres_db_session)):
    """
...
async def calculate_credit_kpi(db_session: Session = Depends(get_postgres_db_session)):
    """
        Calculates credit KPI.
        Returns:
            List[CreditKPI]: Calculated credit KPI.
        """
    result = db_session.execute(db.text(credit_kpi_cmd))
